...
This guide explains how to spin up the full **CodeNest** stack (COC API + Member Backend + Member Frontend) locally using Docker Compose.
...
| `server`   | Built from `docker/Dockerfile.backend`      | `8000` | CodeNest Bun/Express backend   |
| `web`      | Built from `docker/Dockerfile.frontend`     | `5173` | CodeNest React/Vite frontend   |
...
### 2. Member Backend (`docker/.env.local.backend`)

Controls the `server` (CodeNest backend) container:

```env
# ── Auth ─────────────────────────────────────────────────────
JWT_SECRET=COC_MEMBER_JWT_SECRET          # use a strong random value
REFRESH_SECRET=COC_MEMBER_REFRESH_SECRET  # use a strong random value
SALTING=8
...
# ── COC API ──────────────────────────────────────────────────
# Internal Docker network hostname — do NOT change this
# (also set inline by docker-compose.yml as an override)
API_URL=http://coc-api:3000

# ── Server ───────────────────────────────────────────────────
PORT=8000
# IMPORTANT: keep 'production' — refresh tokens (HTTP-only cookies) require it
NODE_ENV='production'
...
RESEND_API_KEY="re_xxxxxxxxxxxxxxxx"       # get from resend.com → API Keys
...
### 3. Member Frontend (`docker/.env.local.frontend`)

Controls the `web` (CodeNest frontend) container:
...
The workflow at `.github/workflows/docker-image.yml` (**Build & Push Docker Images**) automatically builds and pushes both the backend and frontend **production** images to Docker Hub on every push to `main`.
...
| Image                                    | Registry tag                             |
| ---------------------------------------- | ---------------------------------------- |
| `callofcode07/coc-member-backend:latest` | Built from `docker/Dockerfile.backend` (`production` target) |
| `callofcode07/coc-member-frontend:latest`| Built from `docker/Dockerfile.frontend` (`production` target) |
...
- The `coc-api` health-check is failing (bad `DATABASE_URL` or `SUPABASE_*` credentials in `docker/.env.local.coc-api`)
...
- `PORT` and `API_URL` are also set as inline `environment` overrides in `docker-compose.yml` for the `server` service — the compose values take precedence over the env file.
